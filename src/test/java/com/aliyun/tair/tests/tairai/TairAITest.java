...
public class TairAITest extends TairAITestBase {
    ...
    public void taihnswsetTest() {
        ...
        ret = tairAI.set(randomkey_, dim, id1, vector1);
        assertEquals("OK", ret);
        ret = tairAI.set(randomkey_, dim, id2, vector2);
        assertEquals("OK", ret);
        List<Float>  vecret = tairAI.get(randomkey_, id1);
    ...
    public void taihnswmsetTest() throws Exception{
        ...
        ret = tairAI.set(randomkey_, dim, id1, vector1);
        assertEquals("OK", ret);
        ret = tairAI.mset(randomkey_, dim, vectors);
        assertEquals("OK", ret);
        List<Float>  vecret = tairAI.get(randomkey_, id2);
    ...
    public void taihnswwatchTest() {
        ...
        ret = tairAI.set(randomkey_, dim, id1, vector1);
        assertEquals("OK", ret);
        ret = tairAI.watch(randomkey_, dim, "200*");
        jedis.del("2001");
        ...
        try {
            List<Float> vecret = tairAI.get(randomkey_, 2001);
            assertEquals(vecret, tobeverify);
        }
        finally {
        }

        TairAIStatResult statret = tairAI.stat(randomkey_);
    ...
    public void taihnswknngetTest() {
        ...
        ret = tairAI.mset(randomkey_, dim, vectors);
        assertEquals("OK", ret);
        TairAIKnngetResult knnret = tairAI.knnget(randomkey_, 3, vector1);
