...
public class TairVectorClusterTest extends TairVectorTestBase {
    ...
    public void tvs_knnsearch_with_params() {
        ...
        long topn = 10L;
        VectorBuilderFactory.Knn<String> result_string = tairVectorCluster.tvsknnsearch(index, topn,
                "[0.12, 0.23, 0.56, 0.67, 0.78, 0.89, 0.01, 0.89]", ef_params.toArray(new String[0]));
        assertEquals(2, result_string.getKnnResults().size());

        VectorBuilderFactory.Knn<byte[]> entity_byte = tairVectorCluster.tvsknnsearch(SafeEncoder.encode(index), topn,
    ...
    public void tvs_mknnsearch_with_params() {
        ...
        long topn = 10L;
        List<String> vectors = Arrays.asList("[0.12, 0.23, 0.56, 0.67, 0.78, 0.89, 0.01, 0.89]", "[0.22, 0.33, 0.66, 0.77, 0.88, 0.89, 0.11, 0.89]");
        Collection<VectorBuilderFactory.Knn<String>> result_string = tairVectorCluster.tvsmknnsearch(index, topn, vectors, ef_params.toArray(new String[0]));
        assertEquals(2, result_string.size());
        result_string.forEach(one -> System.out.printf("string: %s\n", one.toString()));


        Collection<VectorBuilderFactory.Knn<byte[]>> entity_byte = tairVectorCluster.tvsmknnsearch(SafeEncoder.encode(index), topn,
